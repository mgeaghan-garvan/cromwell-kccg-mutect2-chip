...
if (!(length(args) %in% c(1, 3))) {
  stop(paste(
    ...
    "Usage: Rscript filter_cohort_chip.R <INPUT FILE LIST> <PREVALENCE THRESHOLD> [ALL|NO_FAIL|PASS]",
    ...
    "    [ALL|NO_FAIL|PASS]:    whether to consider all ('ALL') instances of a variant across a cohort when determining prevalence, or only consider passing ('PASS') or non-failing ('NO_FAIL') variants. Default: 'ALL'",
    sep = "\n"))
}
...
if (length(args) == 3) {
  ...
  if (tmp >= 0 && tmp <= 1) {
    ...
    stop(paste("Invalid prevalence threshold supplied:", tmp))
  }
  tmp <- as.character(args[3])
  if (tmp %in% c("ALL", "PASS", "FAIL")) {
    FILTER <- tmp
  } else {
    stop(paste("Invalid argument:", tmp))
  }
}
