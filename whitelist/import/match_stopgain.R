...
match_sg_conditions <- function(conditions, aa_changes, aa_exons, parse_cond_func, parse_aa_change_func) {
  whitelist <- FALSE
  manual_review <- FALSE
...
match_sg <- function(x, parse_cond_func, parse_aa_change_func) {
  whitelist <- FALSE
  manual_review <- FALSE
  ...
  if(grepl("stopgain", x[[1]], perl = TRUE)) {
    ...
    match_conditions <- match_sg_conditions(conditions, aa_changes, aa_exons, parse_cond_func, parse_aa_change_func)
    match_put_conditions <- match_sg_conditions(conditions_put, aa_changes, aa_exons, parse_cond_func, parse_aa_change_func)
